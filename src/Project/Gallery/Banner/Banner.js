...
const Banner=()=>{
      ...
      const home=param.get('home')
      const dispatch=useDispatch()
      const[update,setUpdate]=useState([])

      const Gallery=()=>{

        let x=State.Arraytwo.filter((value,index)=>{
    ...
    return(
        <>
        <Box sx={{width:{md:"100%",xs:"100%",sm:"100%"},flexGrow: 1, display: { xs: 'block', sm: 'block',lg:'block',md:'block'}}} className="Gallery">
